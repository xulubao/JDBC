...
public class Employee {
    ...
    public void menuEmp() throws SQLException, ClassNotFoundException {
        ...
        while (menuState){
            System.out.println("【1】登陆系统\t【2】查询员工信息\t【3】编辑员工信息\t【4】注册员工\t【5】退出登陆\t【0】返回上一层");
            menu = tool.isMenu(0,5);
            switch (menu){
                case 1:
                    this.loginEmployee();
                    break;
    ...
    public void loginEmployee() throws SQLException, ClassNotFoundException {
        ...
        if (this.checkPass(empname,empPass)){
            ...
            this.setEmpId(empId);
            this.state=true;
            System.out.println("登陆成功！欢迎"+empname+"来到魅力四射网吧！");
        }else {
     ...
     * @throws ClassNotFoundException
     */
    public void outEmployee() throws SQLException, ClassNotFoundException {
        if (!this.isEmpState()){
            System.out.println("您没有登陆，不能退出！");
            return;
        }
        this.alterEmpState(empId,2);
        this.setState(false);
